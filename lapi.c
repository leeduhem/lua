...
LUA_API void lua_xmove (lua_State *from, lua_State *to, int n) {
  ...
  api_check(from, to->ci->top - to->top >= n, "stack overflow" AT_LOCATION);
  from->top -= n;
  for (i = 0; i < n; i++) {
...
static UpVal **getupvalref (lua_State *L, int fidx, size_t n, LClosure **pf) {
