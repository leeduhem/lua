...
static l_noret error_expected (LexState *ls, int token) {
  ls->syntax_error(luaO_pushfstring(ls->L, "%s expected", ls->token2str(token)));
}
...
inline void check_condition(LexState *ls, bool c, const char *msg) {
  if (!c) ls->syntax_error(msg);
}
...
inline void leavelevel(LexState *ls) {
  ls->L->nCcalls--;
}
  ...
  if (ls->t != ')') {  /* is 'parlist' not empty? */
    do {
      switch (static_cast<int>(ls->t)) {
        ...
        default: ls->syntax_error("<name> or '...' expected");
      }
...
static void forbody (LexState *ls, int base, int line, int nvars, int isgen) {
  ...
  FuncState *fs = ls->fs;
  checknext(ls, TK_DO);
  int prep = luaK_codeABx(fs, forprep[isgen], base, 0);
  ...
  int endfor = luaK_codeABx(fs, forloop[isgen], base, 0);
...
LClosure *luaY_parser (lua_State *L, ZIO *z, Mbuffer *buff,
  ...
  luaD_inctop(L);
  Table *h = luaH_new(L); // create table for scanner
  sethvalue2s(L, L->top, h); // anchor it
  luaD_inctop(L);
